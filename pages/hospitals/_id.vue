      ...
      <div class="flex flex-wrap -m-4">
        ...
        <div v-for="offer in hospital.offersDetails" class="p-4  xl:w-1/4 md:w-1/2 w-full">
          <div class="h-full p-6 rounded-lg border-2 border-teal-500 bg-black flex flex-col relative overflow-hidden">
            <span class="bg-teal-900 text-white px-5  py-3 tracking-widest text-xs absolute right-0 top-0 rounded-bl ">{{offer.itemPrice}}$</span>
            <h2 v-if="offer.amount===100" class="text-sm tracking-widest title-font mb-1 font-medium">PRO</h2>
            <h1 class="text-5xl text-white leading-none flex items-center pb-4 mb-4 border-b border-gray-200">
              <span>%{{offer.amount}}</span>
              <span class="text-lg ml-1 font-normal text-white"></span>
            </h1>
  ...
  export default {
    ...
    methods: {
      ...
      async checkout(offerId) {
        try {
          const response = await this.$axios.$post(checkoutPath, qs.stringify({
